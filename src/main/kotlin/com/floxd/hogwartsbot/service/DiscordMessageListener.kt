...
@Component
class DiscordMessageListener(val effectService: EffectService) : ListenerAdapter() {
    private val LOGGER = LoggerFactory.getLogger(DiscordMessageListener::class.java)
    override fun onMessageReceived(event: MessageReceivedEvent) {
        try {
            if (effectService.checkForEffect(SpellEnum.INCENDIO, event.author.id).isAffected) {
